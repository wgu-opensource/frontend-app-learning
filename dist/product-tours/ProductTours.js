...
import { closeNewUserCourseHomeModal, endCourseHomeTour, endCoursewareTour, fetchTourData, } from './data';
const ProductTours = ({ activeTab, courseId, isStreakCelebrationOpen, org, }) => {
    const { proctoringPanelStatus, } = useSelector(state => state.courseHome);
    const { showCoursewareTour, showExistingUserCourseHomeTour, showNewUserCourseHomeModal, showNewUserCourseHomeTour, } = useSelector(state => state.tours);
    ...
    return (_jsxs(_Fragment, { children: [_jsx(ProductTour, { tours: tours }), _jsx(NewUserCourseHomeTourModal, { isOpen: isOutlineTab && showNewUserCourseHomeModal, onDismiss: () => {
