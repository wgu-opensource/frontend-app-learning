...
import { checkBlockCompletion, fetchCourse, fetchSequence, getResumeBlock, getSequenceForUnitDeprecated, saveSequencePosition, } from './data';
...
export const checkResumeRedirect = memoize((courseStatus, courseId, sequenceId, firstSequenceId, navigate, isPreview) => {
...
// Look at where this is called in componentDidUpdate for more info about its usage
export const checkSectionUnitToUnitRedirect = memoize((courseStatus, courseId, sequenceStatus, section, unitId, navigate, isPreview) => {
...
// Look at where this is called in componentDidUpdate for more info about its usage
export const checkSectionToSequenceRedirect = memoize((courseStatus, courseId, sequenceStatus, section, unitId, navigate) => {
    if (courseStatus === 'loaded' && sequenceStatus === 'failed' && section && !unitId) {
...
// Look at where this is called in componentDidUpdate for more info about its usage
export const checkUnitToSequenceUnitRedirect = memoize((courseStatus, courseId, sequenceStatus, sequenceMightBeUnit, sequenceId, section, routeUnitId, navigate, isPreview) => {
    if (courseStatus === 'loaded' && sequenceStatus === 'failed' && !section && !routeUnitId) {
        if (sequenceMightBeUnit) {
            ...
            getSequenceForUnitDeprecated(courseId, unitId).then(parentId => {
                if (parentId) {
...
// Look at where this is called in componentDidUpdate for more info about its usage
export const checkSequenceToSequenceUnitRedirect = memoize((courseId, sequenceStatus, sequence, unitId, navigate, isPreview) => {
...
// Look at where this is called in componentDidUpdate for more info about its usage
export const checkSequenceUnitMarkerToSequenceUnitRedirect = memoize((courseId, sequenceStatus, sequence, unitId, navigate, isPreview) => {
    var _a;
    if (sequenceStatus !== 'loaded' || !sequence.id) {
        return;
    }
    const baseUrl = `/course/${courseId}/${sequence.id}`;
    const hasUnits = ((_a = sequence.unitIds) === null || _a === void 0 ? void 0 : _a.length) > 0;
    if (hasUnits) {
        const sequenceUrl = isPreview ? `/preview${baseUrl}` : baseUrl;
        if (unitId === 'first') {
...
class CoursewareContainer extends Component {
    constructor() {
        super(...arguments);
        this.checkSaveSequencePosition = memoize((unitId) => {
        ...
        this.handlePreviousSequenceClick = () => { };
    }
    componentDidMount() {
    ...
    componentDidUpdate() {
        const { courseId, sequenceId, courseStatus, sequenceStatus, sequenceMightBeUnit, sequence, firstSequenceId, sectionViaSequenceId, routeCourseId, routeSequenceId, routeUnitId, navigate, isPreview, } = this.props;
        // Load data whenever the course or sequence ID changes.
        this.checkFetchCourse(routeCourseId);
        this.checkFetchSequence(routeSequenceId);
    ...
    render() {
        ...
        return (_jsx(TabPage, Object.assign({ activeTabSlug: "courseware", courseId: courseId, unitId: routeUnitId, courseStatus: courseStatus, metadataModel: "coursewareMeta" }, { children: _jsx(Course, { courseId: courseId, sequenceId: sequenceId, unitId: routeUnitId, nextSequenceHandler: this.handleNextSequenceClick, previousSequenceHandler: this.handlePreviousSequenceClick, unitNavigationHandler: this.handleUnitNavigationClick }) })));
    }
}
...
const currentCourseSelector = createSelector((state) => state.models.coursewareMeta || {}, (state) => state.courseware.courseId, (coursesById, courseId) => (coursesById[courseId] ? coursesById[courseId] : null));
const currentSequenceSelector = createSelector((state) => state.models.sequences || {}, (state) => state.courseware.sequenceId, (sequencesById, sequenceId) => (sequencesById[sequenceId] ? sequencesById[sequenceId] : null));
const sequenceIdsSelector = createSelector((state) => state.courseware.courseStatus, currentCourseSelector, (state) => state.models.sections, (courseStatus, course, sectionsById) => {
...
const previousSequenceSelector = createSelector(sequenceIdsSelector, (state) => state.models.sequences || {}, (state) => state.courseware.sequenceId, (sequenceIds, sequencesById, sequenceId) => {
...
const nextSequenceSelector = createSelector(sequenceIdsSelector, (state) => state.models.sequences || {}, (state) => state.courseware.sequenceId, (sequenceIds, sequencesById, sequenceId) => {
    if (!sequenceId || sequenceIds.length === 0) {
        return null;
    }
    const sequenceIndex = sequenceIds.indexOf(sequenceId);
...
const firstSequenceIdSelector = createSelector((state) => state.courseware.courseStatus, currentCourseSelector, (state) => state.models.sections || {}, (courseStatus, course, sectionsById) => {
    if (courseStatus !== 'loaded') {
        return null;
    }
    const { sectionIds = [] } = course;
...
const sectionViaSequenceIdSelector = createSelector((state) => state.models.sections || {}, (state) => state.courseware.sequenceId, (sectionsById, sequenceId) => (sectionsById[sequenceId] ? sectionsById[sequenceId] : null));
const mapStateToProps = (state) => {
    const { courseId, sequenceId, courseStatus, sequenceStatus, sequenceMightBeUnit, } = state.courseware;
    return {
        courseId,
        sequenceId,
        courseStatus,
