...
const NotesVisibility = ({ course }) => {
    ...
    const handleClick = () => {
        ...
        getAuthenticatedHttpClient().put(visibilityUrl, data).then(() => {
            setVisible(!visible);
            toggleNotes();
        });
    };
    const message = visible ? 'notes.button.hide' : 'notes.button.show';
    return (_jsxs("button", Object.assign({ className: `trigger btn ${visible ? 'text-secondary' : 'text-success'}  mx-2 `, role: "switch", type: "button", onClick: handleClick, onKeyDown: handleClick, tabIndex: "-1", "aria-checked": visible ? 'true' : 'false' }, { children: [_jsx(FontAwesomeIcon, { icon: faPencilAlt, "aria-hidden": "true", className: "mr-2" }), intl.formatMessage(messages[message])] })));
};
