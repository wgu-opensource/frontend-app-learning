...
const CourseExit = () => {
    ...
    const { certificateData, courseExitPageIsActive, courseGoals, enrollmentMode, hasScheduledContent, isEnrolled, userHasPassingGrade, } = useModel('coursewareMeta', courseId);
    const { isMasquerading, canViewCertificate, } = useModel('courseHomeMeta', courseId);
    const mode = getCourseExitMode(certificateData, hasScheduledContent, isEnrolled, userHasPassingGrade, courseExitPageIsActive, canViewCertificate);
    ...
    if (mode === COURSE_EXIT_MODES.nonPassing) {
        body = (_jsx(CourseNonPassing, {}));
    }
    else if (mode === COURSE_EXIT_MODES.inProgress) {
        body = (_jsx(CourseInProgress, {}));
    }
    else if (mode === COURSE_EXIT_MODES.celebration) {
        body = (_jsx(CourseCelebration, {}));
    }
    else {
        return (_jsx(Navigate, { to: `/course/${courseId}`, replace: true }));
    }
    return (_jsxs(_Fragment, { children: [_jsx(CourseExitViewCoursesPluginSlot, {}), body] }));
};
