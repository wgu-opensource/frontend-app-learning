...
function getCourseExitMode(certificateData, hasScheduledContent, isEnrolled, userHasPassingGrade, courseExitPageIsActive = null, canImmediatelyViewCertificate = false, entranceExamPassed = null) {
...
function GetCourseExitNavigation(courseId, intl) {
    const { certificateData, hasScheduledContent, isEnrolled, userHasPassingGrade, courseExitPageIsActive, entranceExamData: { entranceExamPassed }, } = useModel('coursewareMeta', courseId);
    const { canViewCertificate } = useModel('courseHomeMeta', courseId);
    const exitMode = getCourseExitMode(certificateData, hasScheduledContent, isEnrolled, userHasPassingGrade, courseExitPageIsActive, canViewCertificate, entranceExamPassed);
...
const logClick = (org, courseId, administrator, event, extraProperties) => {
    ...
    sendTrackEvent(`edx.ui.lms.course_exit.${event}.clicked`, Object.assign({ org_key: org, courserun_key: courseId, is_staff: administrator }, extraProperties));
};
...
export { COURSE_EXIT_MODES, getCourseExitMode, GetCourseExitNavigation, logClick, logVisit, };
