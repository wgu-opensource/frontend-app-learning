...
const CELEBRATION_LOCAL_STORAGE_KEY = 'CelebrationModal.showOnSectionLoad'; // Records clicks through the end of a section, so that we can know whether we should celebrate when we finish loading
...
function shouldCelebrateOnSectionLoad(courseId, sequenceId, celebrateFirstSection, dispatch, celebrations) {
  ...
  if (sequenceId !== prevSequenceId && !onTargetSequence) {
    ...
    (0, _localStorage.clearLocalStorage)(CELEBRATION_LOCAL_STORAGE_KEY); // Update our local copy of course data from LMS
