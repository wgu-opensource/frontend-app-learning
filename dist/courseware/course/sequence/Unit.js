...
const Unit = ({ courseId, format, onLoaded, id, intl, }) => {
    ...
    return (_jsxs("div", Object.assign({ className: "unit" }, { children: [_jsx("h1", Object.assign({ className: "mb-0 h3" }, { children: unit.title })), _jsx("h2", Object.assign({ className: "sr-only" }, { children: intl.formatMessage(messages.headerPlaceholder) })), _jsx(BookmarkButton, { unitId: unit.id, isBookmarked: unit.bookmarked, isProcessing: unit.bookmarkedUpdateState === 'loading' }), (window.expSocialShareAboutUrls && window.expSocialShareAboutUrls[unit.id] !== undefined) && (_jsx(ShareButton, { url: window.expSocialShareAboutUrls[unit.id] })), contentTypeGatingEnabled && unit.containsContentTypeGatedContent && (_jsx(Suspense, Object.assign({ fallback: (_jsx(PageLoading, { srMessage: intl.formatMessage(messages.loadingLockedContent) })) }, { children: _jsx(LockPaywall, { courseId: courseId }) }))), shouldDisplayHonorCode && (_jsx(Suspense, Object.assign({ fallback: (_jsx(PageLoading, { srMessage: intl.formatMessage(messages.loadingHonorCode) })) }, { children: _jsx(HonorCode, { courseId: courseId }) }))), !shouldDisplayHonorCode && !hasLoaded && !showError && (_jsx(PageLoading, { srMessage: intl.formatMessage(messages.loadingSequence) })), !shouldDisplayHonorCode && !hasLoaded && showError && (_jsx(ErrorPage, {})), modalOptions.open && (_jsx(Modal, { body: (_jsx(_Fragment, { children: modalOptions.body
                        ? _jsx("div", Object.assign({ className: "unit-modal" }, { children: modalOptions.body }))
                        : (_jsx("iframe", { title: modalOptions.title, allow: IFRAME_FEATURE_POLICY, frameBorder: "0", src: modalOptions.url, style: {
                                width: '100%',
                                height: '100vh',
                            } })) })), onClose: () => { setModalOptions({ open: false }); }, open: true, dialogClassName: "modal-lti" })), !shouldDisplayHonorCode && (_jsx("div", Object.assign({ className: "unit-iframe-wrapper" }, { children: _jsx("iframe", { id: "unit-iframe", title: unit.title, src: iframeUrl, allow: IFRAME_FEATURE_POLICY, allowFullScreen: true, height: iframeHeight, scrolling: "no", referrerPolicy: "origin", onLoad: () => {
