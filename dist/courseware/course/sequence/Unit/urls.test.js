...
describe('urls module getIFrameUrl', () => {
    ...
    test('jumpToId and fragmentIdentifier is added to url', () => {
        expect(getIFrameUrl(Object.assign(Object.assign({}, props), { jumpToId: 'some-xblock-id' }))).toEqual('https://test-lms-url/xblock/test-id?exam_access=test-access-token&format=test-format&jumpToId=some-xblock-id&preview=false&recheck_access=1&show_bookmark=0&show_title=0&view=test-view#some-xblock-id');
    });
    test('preview is true and url param equals 1', () => {
        expect(getIFrameUrl(Object.assign(Object.assign({}, props), { preview: true }))).toEqual('https://test-lms-url/xblock/test-id?exam_access=test-access-token&format=test-format&preview=true&recheck_access=1&show_bookmark=0&show_title=0&view=test-view');
    });
});
