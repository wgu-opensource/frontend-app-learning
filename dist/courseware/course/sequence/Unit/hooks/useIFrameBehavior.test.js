...
jest.mock('react', () => (Object.assign(Object.assign({}, jest.requireActual('react')), { useCallback: jest.fn((cb, prereqs) => ({ cb, prereqs })) })));
...
const mockState = (state) => {
    ...
    if ('iframeHeight' in state) {
        jest.spyOn(iframeBehaviorState, 'iframeHeight').mockImplementation(() => [iframeHeight, setIframeHeight]);
    }
    if ('hasLoaded' in state) {
        jest.spyOn(iframeBehaviorState, 'hasLoaded').mockImplementation(() => [hasLoaded, setHasLoaded]);
    }
    if ('showError' in state) {
        jest.spyOn(iframeBehaviorState, 'showError').mockImplementation(() => [showError, setShowError]);
    }
    if ('windowTopOffset' in state) {
        jest.spyOn(iframeBehaviorState, 'windowTopOffset').mockImplementation(() => [windowTopOffset, setWindowTopOffset]);
    }
};
describe('useIFrameBehavior hook', () => {
    ...
    describe('behavior', () => {
        ...
        describe('effects - on frame change', () => {
            ...
            beforeEach(() => {
                var _a;
                (_a = global.window) !== null && _a !== void 0 ? _a : (global.window = Object.create(window));
            ...
            describe('resize message', () => {
                ...
                describe('hasLoaded', () => {
                    it('sets iframe height with payload height', () => {
                        mockState(Object.assign(Object.assign({}, defaultStateVals), { hasLoaded: true }));
                        renderHook(() => useIFrameBehavior(props));
                        const { cb } = useEventListener.mock.calls[0][1];
                ...
                describe('payload is present but uninitialized', () => {
                    ...
                    test('onLoaded is optional', () => {
                        renderHook(() => useIFrameBehavior(Object.assign(Object.assign({}, props), { onLoaded: undefined })));
                        const { cb } = useEventListener.mock.calls[0][1];
                        cb(resizeMessage());
                        expect(setHasLoaded).toHaveBeenCalledWith(true);
                    });
                ...
                it('scrolls to current window vertical offset if one is set', () => {
                    ...
                    mockState(Object.assign(Object.assign({}, defaultStateVals), { windowTopOffset }));
                    renderHook(() => useIFrameBehavior(props));
                    const { cb } = useEventListener.mock.calls[0][1];
        ...
        describe('visibility tracking', () => {
            it('sets up visibility tracking after iframe loads', () => {
                ...
                expect(postMessage).not.toHaveBeenCalledWith(expect.objectContaining({ type: 'unit.visibilityStatus' }), config.LMS_BASE_URL);
            });
            it('does not set up visibility tracking before iframe has loaded', () => {
                window.location.hash = ''; // Avoid posting hash message.
                mockState(Object.assign(Object.assign({}, defaultStateVals), { hasLoaded: false }));
                renderHook(() => useIFrameBehavior(props));
