...
Factory.define('courseBlocks')
    ...
    Factory.build('block', { type: 'sequential', children: getIds(units) }, { courseId }),
])
    .option('sections', ['courseId', 'sequences'], (courseId, sequences) => [
    Factory.build('block', { type: 'chapter', children: getIds(sequences) }, { courseId }),
])
    .option('course', ['courseId', 'sections'], (courseId, sections) => Factory.build('block', { type: 'course', children: getIds(sections) }, { courseId }))
    .attr('blocks', ['course', 'sections', 'sequences', 'units'], (course, sections, sequences, units) => (Object.assign(Object.assign(Object.assign({ [course.id]: course }, getBlocks(sections)), getBlocks(sequences)), getBlocks(units))))
...
export function buildSimpleCourseBlocks(courseId, title, options = {}) {
    const unitBlocks = options.unitBlocks || [Factory.build('block', { type: 'vertical' }, { courseId })];
    const sequenceBlocks = options.sequenceBlocks || [Factory.build('block', { type: 'sequential', children: unitBlocks.map(block => block.id) }, { courseId })];
    const sectionBlocks = options.sectionBlocks || [Factory.build('block', { type: 'chapter', children: sequenceBlocks.map(block => block.id) }, { courseId })];
    const courseBlock = options.courseBlock || Factory.build('block', { type: 'course', display_name: title, children: sectionBlocks.map(block => block.id) }, { courseId });
    return {
        courseBlocks: options.courseBlocks || Factory.build('courseBlocks', {
            courseId,
...
export function buildMinimalCourseBlocks(courseId, title, options = {}) {
    const sequenceBlocks = options.sequenceBlocks || [Factory.build('block', {
            display_name: 'Title of Sequence',
            effort_activities: 2,
            effort_time: 15,
            type: 'sequential',
        }, { courseId })];
    const sectionBlocks = options.sectionBlocks || [Factory.build('block', {
            type: 'chapter',
    ...
    const courseBlock = options.courseBlock || Factory.build('block', {
        type: 'course',
        display_name: title,
    ...
    return {
        courseBlocks: options.courseBlocks || Factory.build('courseBlocks', { courseId }, {
            sequences: sequenceBlocks,
            sections: sectionBlocks,
            course: courseBlock,
            units: [],
...
export function buildBinaryCourseBlocks(courseId, title) {
    ...
    [0, 1].forEach(sectionIndex => {
        [0, 1].forEach(sequenceIndex => {
            [0, 1].forEach(unitIndex => {
                unitTree[sectionIndex][sequenceIndex][unitIndex] = Factory.build('block', { type: 'vertical' }, { courseId });
            });
            sequenceTree[sectionIndex][sequenceIndex] = Factory.build('block', { type: 'sequential', children: unitTree[sectionIndex][sequenceIndex].map(block => block.id) }, { courseId });
        });
        sectionTree[sectionIndex] = Factory.build('block', { type: 'chapter', children: sequenceTree[sectionIndex].map(block => block.id) }, { courseId });
    });
    const courseBlock = Factory.build('block', { type: 'course', display_name: title, children: sectionTree.map(block => block.id) }, { courseId });
    const sectionBlocks = [
        sectionTree[0],
        sectionTree[1],
    ];
    ...
    return {
        ...
        // work with.
        courseBlocks: Factory.build('courseBlocks', { courseId, title }, {
            units: unitBlocks,
            sequences: sequenceBlocks,
            sections: sectionBlocks,
            course: courseBlock,
