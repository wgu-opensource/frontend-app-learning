...
const ROUTES = [].concat(...Object.values(DECODE_ROUTES).map(value => (Array.isArray(value) ? value : [value])));
...
const DecodePageRoute = ({ children }) => {
    ...
    if (computedMatch) {
        ...
        if (newUrl !== pathname) {
            return _jsx(Navigate, { to: newUrl, replace: true });
        }
    }
    return _jsxs(PageWrap, { children: [" ", children, " "] });
};
