...
function useCertificateStatusAlert(courseId) {
    ...
    const { isEnrolled, org, tabs, } = useModel('courseHomeMeta', courseId);
    const { datesWidget: { courseDateBlocks, }, certData, hasEnded, userHasPassingGrade, userTimezone, enrollmentMode, } = useModel('outline', courseId);
    const { certStatus, certWebViewUrl, certificateAvailableDate, } = certData || {};
    const endBlock = courseDateBlocks.find(b => b.dateType === 'course-end-date');
    const isVerifiedEnrollmentMode = (enrollmentMode !== null
    ...
    const notPassingCourseEnded = (isEnrolled
