...
const CertificateStatusAlert = ({ payload }) => {
    ...
    const { certificateAvailableDate, certStatus, courseEndDate, courseId, certURL, userTimezone, org, notPassingCourseEnded, tabs, } = payload;
    ...
    const renderCertAwardedStatus = () => {
        ...
        if (certStatus === CERT_STATUS_TYPE.EARNED_NOT_AVAILABLE) {
            ...
            const certificateAvailableDateFormatted = _jsx(FormattedDate, { value: certificateAvailableDate, day: "numeric", month: "long", year: "numeric" });
            const courseEndDateFormatted = _jsx(FormattedDate, { value: courseEndDate, day: "numeric", month: "long", year: "numeric" });
            alertProps.header = intl.formatMessage(certMessages.certStatusEarnedNotAvailableHeader);
            alertProps.body = (_jsx("p", { children: _jsx(FormattedMessage, Object.assign({ id: "learning.outline.alert.cert.earnedNotAvailable", defaultMessage: "This course ends on {courseEndDateFormatted}. Final grades and any earned certificates are\n            scheduled to be available after {certificateAvailableDate}.", values: {
    ...
    return (_jsx(AlertWrapper, Object.assign({}, alertProps, { children: ({ variant, buttonVisible, iconClassName, icon, header, body, buttonAction, buttonLink, buttonMessage, }) => (_jsx(Alert, Object.assign({ variant: variant }, { children: _jsxs("div", Object.assign({ className: "d-flex flex-column flex-lg-row justify-content-between align-items-center" }, { children: [_jsxs("div", Object.assign({ className: buttonVisible ? 'col-lg-8' : 'col-auto' }, { children: [_jsx(FontAwesomeIcon, { icon: icon, className: iconClassName }), _jsx(Alert.Heading, { children: header }), body] })), buttonVisible && (_jsx("div", Object.assign({ className: "flex-grow-0 pt-3 pt-lg-0" }, { children: _jsx(Button, Object.assign({ variant: "primary", href: buttonLink, onClick: () => {
                                if (buttonAction) {
                                    buttonAction();
                                }
