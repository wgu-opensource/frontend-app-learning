...
const LearningGoalButton = ({ level, isSelected, handleSelect, }) => {
    const intl = useIntl();
    const buttonDetails = {
        casual: {
            daysPerWeek: 1,
            ...
            icon: _jsx(FlagCasualIcon, {}),
        },
        regular: {
            daysPerWeek: 3,
            title: messages.regularGoalButtonTitle,
            text: messages.regularGoalButtonText,
            icon: _jsx(FlagRegularIcon, {}),
        },
        intense: {
            daysPerWeek: 5,
            title: messages.intenseGoalButtonTitle,
            text: messages.intenseGoalButtonText,
            icon: _jsx(FlagIntenseIcon, {}),
        },
    };
    const values = buttonDetails[level];
    return (_jsx(FlagButton, { buttonIcon: values.icon, title: intl.formatMessage(values.title), text: intl.formatMessage(values.text), handleSelect: () => handleSelect(values.daysPerWeek), isSelected: isSelected }));
};
