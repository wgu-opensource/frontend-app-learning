...
describe('Data layer integration tests', () => {
  ...
  describe('Test fetchDatesTab', () => {
    ...
    it.each([401, 403, 404])('should result in fetch denied if course access is denied, regardless of dates API status', async errorStatus => {
  ...
  describe('Test fetchOutlineTab', () => {
    ...
    it.each([401, 403, 404])('should result in fetch denied if course access is denied, regardless of outline API status', async errorStatus => {
      axiosMock.onGet(courseMetadataUrl).reply(200, courseHomeAccessDeniedMetadata);
  ...
  describe('Test fetchCoursewareSearchSettings', () => {
    it('Should return enabled as true when enabled', async () => {
      const apiUrl = `${(0, _frontendPlatform.getConfig)().LMS_BASE_URL}/courses/${courseId}/courseware-search/enabled/`;
      axiosMock.onGet(apiUrl).reply(200, {
        enabled: true
      });
      const {
        enabled
      } = await thunks.fetchCoursewareSearchSettings(courseId);
