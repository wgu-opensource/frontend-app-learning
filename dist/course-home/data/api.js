...
exports.getCoursewareSearchEnabled = getCoursewareSearchEnabled;
...
exports.searchCourseContentFromAPI = searchCourseContentFromAPI;
...
async function getProgressTabData(courseId, targetUserId) {
  let url = `${(0, _frontendPlatform.getConfig)().LMS_BASE_URL}/api/course_home/progress/${courseId}`;
...
async function getProctoringInfoData(courseId, username) {
  ...
  if (!(0, _frontendPlatform.getConfig)().EXAMS_BASE_URL) {
    url = `${(0, _frontendPlatform.getConfig)().LMS_BASE_URL}/api/edx_proctoring/v1/user_onboarding/status?is_learning_mfe=true&course_id=${encodeURIComponent(courseId)}`;
    if (username) {
      url += `&username=${encodeURIComponent(username)}`;
    }
  } else {
    url = `${(0, _frontendPlatform.getConfig)().EXAMS_BASE_URL}/api/v1/student/course_id/${encodeURIComponent(courseId)}/onboarding`;
    if (username) {
      url += `?username=${encodeURIComponent(username)}`;
    }
  }
...
async function getOutlineTabData(courseId) {
  ...
  let tabData;
  try {
    tabData = await (0, _auth.getAuthenticatedHttpClient)().get(url);
  } catch (error) {
    const httpErrorStatus = error?.response?.status;
    if (httpErrorStatus === 403) {
      // The backend sends this if there is a course access error and the user should be redirected. The redirect
...
async function getCoursewareSearchEnabled(courseId) {
  const url = new URL(`${(0, _frontendPlatform.getConfig)().LMS_BASE_URL}/courses/${courseId}/courseware-search/enabled/`);
  const {
    data
...
async function searchCourseContentFromAPI(courseId, searchKeyword, options = {}) {
  const defaults = {
    page: 0,
    limit: 20
  };
  ...
  const url = new URL(`${(0, _frontendPlatform.getConfig)().LMS_BASE_URL}/search/${courseId}`);
  const formData = `search_string=${searchKeyword}&page_size=${limit}&page_index=${page}`;
  const response = await (0, _auth.getAuthenticatedHttpClient)().post(url.href, formData);
  return (0, _frontendPlatform.camelCaseObject)(response);
}
