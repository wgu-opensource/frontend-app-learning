...
const ShiftDatesAlert = ({ fetch, model }) => {
    ...
    const { missedDeadlines, missedGatedContent, } = datesBannerInfo;
    const dispatch = useDispatch();
    if (!missedDeadlines || missedGatedContent || hasEnded) {
        return null;
    }
    return (_jsx(Alert, Object.assign({ variant: "warning" }, { children: _jsxs(Row, Object.assign({ className: "w-100 m-0" }, { children: [_jsxs(Col, Object.assign({ xs: 12, md: 9, className: "small p-0 pr-md-2" }, { children: [_jsx("strong", { children: intl.formatMessage(messages.missedDeadlines) }), ' ', intl.formatMessage(messages.shiftDatesBody)] })), _jsx(Col, Object.assign({ xs: 12, md: 3, className: "align-self-center text-right mt-3 mt-md-0 p-0" }, { children: _jsx(Button, Object.assign({ variant: "primary", size: "sm", className: "w-xs-100 w-md-auto", onClick: () => dispatch(resetDeadlines(courseId, model, fetch)) }, { children: intl.formatMessage(messages.shiftDatesButton) })) }))] })) })));
};
